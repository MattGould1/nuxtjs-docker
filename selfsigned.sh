...
build() {
  ...
  FILENAME=${CN/\*\./}
  # Generate CA key & crt
  openssl genrsa -out ${tmp}/tmp.pem 2048
  openssl req -x509 -new -nodes -key ${tmp}/tmp.pem -sha256 -days ${DURATION} -out ${OUTPATH}${FILENAME}_CA.pem -subj "/C=${C}/ST=${ST}/L=${L}/O=${O}/OU=${OU}/CN=${CN}/emailAddress=${emailAddress}"
  ...
  # Server key
  openssl req -new -sha256 -nodes -out ${OUTPATH}${FILENAME}.csr -newkey rsa:2048 -keyout ${OUTPATH}${FILENAME}.key.pem -config <( cat ${tmp}/tmp.csr.cnf )

  # Server certificate
  openssl x509 -req -in ${OUTPATH}${FILENAME}.csr -CA ${OUTPATH}${FILENAME}_CA.pem -CAkey ${tmp}/tmp.pem -CAcreateserial -out ${OUTPATH}${FILENAME}.crt.pem -days ${DURATION} -sha256 -extfile ${tmp}/v3.ext
}
